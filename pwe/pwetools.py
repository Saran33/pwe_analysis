...
def read_pd_csv(f_path, low_memory=False, index_col=['DateTime'], parse_dates=['DateTime'], infer_datetime_format=True):
    df = pd.read_csv(f_path, low_memory=low_memory, index_col=index_col,
...
def check_for_recent(dir='csv_files', file_type='csv', horizon='today'):
    ...
    if os.path.exists(abs_path) == True:
        ...
        while True:
            try:
                mk_new_dir = input(
                    "Would you like to create one? ").strip().strip('"').strip("'")
...
def find_recent(dir='csv_files', file_type='csv', horizon='today'):
    ...
    """
    if horizon == 'now':
        latest_file = check_for_recent(
            dir=dir, file_type=file_type, horizon='now')

    elif horizon == 'today':
        latest_file = check_for_recent(
            dir=dir, file_type=file_type, horizon='today')

    elif horizon == 'yesterday':
        latest_file = check_for_recent(
            dir=dir, file_type=file_type, horizon='yesterday')

    elif horizon == 'week':
        latest_file = check_for_recent(
            dir=dir, file_type=file_type, horizon='week')

    elif horizon == 'any':
        latest_file = check_for_recent(
            dir=dir, file_type=file_type, horizon='any')
    ...
    if latest_file == 'NA':
        ...
        while True:
            try:
                if latest_file == 'NA':
                    get_older = input(
                        "Would you like to try for an older file? Y/N? ").strip().strip('"')

                    if get_older.lower().startswith('y'):
                        print("What horizon would you like to try for?")
                        wht_hzn = input(
                            "today, yesterday, week or any? Or Q to quit.  ").strip().strip('"')

                        if wht_hzn.lower().startswith('t'):
                            hrzn = 'today'
                            latest_file = find_recent(
                                dir=dir, file_type=file_type, horizon=hrzn)
                            if latest_file == 'NA':
                                print(
                                    f"No local file exists for the horizon: '{hrzn}'")
                                continue
                            elif latest_file != 'NA':
                                return latest_file
                ...
                print(error)
                print(
                    'Please enter "today,"(t) "yesterday,"(y) "week"(w) or "any"(a). Or "Q" to quit.')
                print("")
                continue
    ...
    while True:
        ...
        if type(arg) is str:
            ...
            if arg.index.tzinfo == None:
                ...
                print("Index is already assigned to the timezone:",
                      str(arg.index.tzinfo))
                print("To convert it to a different timezone, use: change_tz")
                break
...
def format_ohlc(df):
    if 'Close' in df:
        ...
        df.columns = df.columns.str.strip().str.capitalize().str.replace(
...
def nan_percent(df):
    ...
    nan_percent = pd.Series(["{0:.2%}".format(val)
                            for val in nan_percent], index=nan_percent.index)
    return nan_percent
...
def resample_ohlc(df, interval='1H', origin='start_day', offset=None, o='Open', h='High', l='Low', c='Close', v='Volume',
    ...
    for x in ohlc_dict:
        # offset=0
        df = df.resample(interval, origin=origin, offset=offset).apply(
            ohlc_dict)  # origin=0
...
def first_day_of_current_year(time=False, utc=False):
    ...
    """
    if time:
        fdocy = datetime.now().replace(month=1, day=1, hour=0,
...
def last_day_of_current_year(time=False, utc=False):
    ...
    """
    if time:
        ldocy = datetime.now().replace(month=12, day=31, hour=23,
...
def get_unique_dates(df, yearfirst=True, dayfirst=True):
    ...
    unique = pd.DataFrame(uni.index.unique(),
                          index=uni.index.unique(), columns=['Unique Date'])
    unique.index = pd.to_datetime(
        unique.index, yearfirst=yearfirst, dayfirst=dayfirst)
...
def count_df_dupes_idx(df, keep='first'):
    ...
    if dupes > 0:
        ...
        print(f"Duplicate rows: {dupes}", "({:.6%})".format(
            dupe_perc), f"between {dupe_start} and {dupe_end}")
    elif dupes == 0:
...
def concat_non_dupe_idx(df1, df2):
    ...
    if ((df1 is not None and not isinstance(df1, pd.core.frame.DataFrame)) or (df2 is not None and not isinstance(df2, pd.core.frame.DataFrame))):
        raise ValueError(
            'df1 and df2 must be of type pandas.core.frame.DataFrame.')
...
def get_missing_dates(df):
    if type(df.index) != 'DatetimeIndex':
        missing_dates = pd.date_range(
            start=df.index.min(), end=df.index.max()).difference(df.index)
        missing_dates = pd.DataFrame(missing_dates, columns=[
                                     'Missing DateTime'], index=pd.DatetimeIndex(missing_dates))
...
def common_missing_dates(df1, df2):
    ...
    common = df1_miss.loc[(df1_miss.index.astype(
        str).isin(df2_miss.index.astype(str)))]
    common = common.rename(
        columns={'Missing DateTime': 'Common Missing DateTime'})
    ...
    common_dates = common_dates.rename(
        columns={'Unique Date': 'Common Missing Date'})
            ...
            if expires == 'last_fri':
                ...
                if (time == None) or (freq == 'D'):
                    ...
                    dates_to_remove = last_fris_df.loc[(
                        ~last_fris_df['Date'].astype(str).isin(range_df['Date'].astype(str)))]
                    hist_exp_df = last_fris_df.loc[(~last_fris_df['Date'].astype(
                        str).isin(dates_to_remove['Date'].astype(str)))]
                    ...
                    dates_to_remove = last_fris_df.loc[(~last_fris_df['DateTime'].astype(
                        str).isin(range_df['DateTime'].astype(str)))]
                    hist_exp_df = last_fris_df.loc[(~last_fris_df['DateTime'].astype(
                        str).isin(dates_to_remove['DateTime'].astype(str)))]
