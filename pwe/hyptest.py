        ...
        ##############################################################
            ...
            if same.lower() in ('y', 'yes'):
                ...
                if n_dist == None:
                    print("")
                    print(
                        "Are the distributions long-tailed or contaminated with outliers? Y/N?")
                    tails = str(input())
...
def ttest_ind_(group1, group2, H_1='two-sided', median=False,
    ...
    if group1_med < group2_med:
        ...
        print(
            f"The mean of {group1_name} is {diff} lower than the mean of {group2_name}.")
    if group1_med > group2_med:
        diff = "{:.2%}".format(abs((group1_med-group2_med)/group2_med))
        print(
            f"The mean of {group1_name} is {diff} higher than the mean of {group2_name}.")
    print("")
...
def ttest_1samp_(sample, pop, H_1='less', sample_name=None, pop_name=None, nan_policy='omit',):
    ...
    tstat, pval = ttest_1samp(
        sample, pop_mean, nan_policy=nan_policy, alternative=H_1)
    print('p-value:', "{:.6}".format(pval))
    print('t-stat:', "{:.6}".format(tstat))
...
def wilcoxon_sr(group1, group2, H_1='two-sided',
    ...
    if group1_med < group2_med:
        diff = "{:.2%}".format(abs((group2_med-group1_med)/group2_med))
        print(
            f"The median of {group1_name} is {diff} lower than the mean of {group2_name}.")
    if group1_med > group2_med:
        diff = "{:.2%}".format(abs((group1_med-group2_med)/group2_med))
        print(
            f"The median of {group1_name} is {diff} higher than the mean of {group2_name}.")
    print("")
    ...
    if group1.count() == group2.count():
        w, pval = wilcoxon(group1, group2, zero_method=zero_method,
                           correction=correction, alternative=H_1, mode=mode)
    else:
        w, pval = wilcoxon(group1-group2_med, zero_method=zero_method,
                           correction=correction, alternative=H_1, mode=mode)
...
def man_whitney_u(group1, group2, H_1='two-sided', group1_name=None, group2_name=None,
    ...
    u, pval = mannwhitneyu(group1.dropna(), group2.dropna(
    ), use_continuity=use_continuity, alternative=H_1, axis=axis, method=method)
...
def pval_table(prefix_strs, series_names=['2018/01/01-2021/09/01', '2019/09/01-2021/09/01'], interval='Hourly', p_decimals=2, ret_decimals=6):
    ...
    for prefix_str, series_name in zip(prefix_strs, series_names):
        ...
        series_name_lst = ['T-72 to T-48', 'T-48 to T-0', 'T-48 to T-24',
                           'T-24 to T-0', 'T-0', 'T-0 to T+24', 'T+24 to T+48', 'T+48 to T+72']
        ...
        table_df[f'Mean {interval} Return'] = pd.Series(
            [round(val, 6) for val in ret_dict.values()], index=table_df.index)
        table_df[f'Mean {interval} Return'] = pd.Series(
            [f"{val*100:.{ret_decimals}f}%" for val in table_df[f'Mean {interval} Return']], index=table_df.index)

        ret_dict = OrderedDict()
        for key in delta_dict.keys():
            ret_dict[key] = delta_dict[key].ab_ret
        table_df['Abnormal Return'] = pd.Series(
            [val for val in ret_dict.values()], index=table_df.index)
        ...
        table_df['P-value'] = pd.Series([round(val, 6)
                                        for val in pval_dict.values()], index=table_df.index)
        table_df['P-value'] = pd.Series(
            [f"{val*100:.{p_decimals}f}%" for val in table_df['P-value']], index=table_df.index)
        # table_df[f'{series_name} Abnormal {interval} Return'] =

        df_dict[series_name] = table_df

        df_total = pd.concat([df_dict[df] for df in df_dict.keys()], axis=1, keys=[
                             df for df in df_dict.keys()])
