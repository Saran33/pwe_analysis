...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 13 19:29:33 2021
...
def ama(df,window=9,fast_period=6,slow_period=12,price='Close',fillna=False):
    ...
    ## === ta ====
    df[f"AMA_{fast_period}_{slow_period}"]= ta.momentum.KAMAIndicator(df[price],window=window,pow1=6,pow2=12, fillna=fillna).kama()
...
def bol_bands(df,window=20,std=2,price='Close',fillna=True, lib=False, inputs=None):
    ...
    if lib:
        if inputs!=None:
            df[f"bbu_{std}_{window}"], df[f"bbm_{std}_{window}"], df[f"bbl_{std}_{window}"] = abstract.BBANDS(inputs, timeperiod=window, nbdevup=std, nbdevdn=std)
        else:
            df[f"bbu_{std}_{window}"], df[f"bbm_{std}_{window}"], df[f"bbl_{std}_{window}"] = talib.BBANDS(df['Close'].values, timeperiod=10, nbdevup=2, nbdevdn=2)
    ## === /talib ==== 

    ## === ta ====
    else:
        indicator_bb = ta.volatility.BollingerBands(close=df[price], window=window, window_dev=std, fillna=fillna)
...
def adx(df, high='High', low='Low', close='Close', window=14, fillna=True,lib=False, inputs=None):
    ...
    ## === talib ====
        if inputs!=None:
            ...
            df[f"ADX_{window}"] = talib.ADX(df[high],df[low],df[close],timeperiod=window)
    else:
    ## === ta ====
